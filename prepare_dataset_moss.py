...
from utils.llm_utilis import get_embeddings, generate_verilog_prompt, generate_and_prompt, generate_not_prompt
...
if __name__ == '__main__':
    ...
    for bench_filename in name_list[args.start_idx: min(args.end_idx, len(name_list))]:
        ...
        embed_list = []
        for data in x_data:
            if data[1] == gate_to_index['DFF']:
                dff_name = f"_{count:03d}_"
                count += 1
                verilog_path = os.path.join(args.aig_folder, '../experimentB_verilog', circuit_name + '.v')
                texts = generate_verilog_prompt(verilog_path, dff_name)
                embed = get_embeddings(texts, None)
                embed_list.append(embed.squeeze(0).numpy().tolist())
            elif data[1] == gate_to_index['AND']:
                texts = generate_and_prompt()
                embed = get_embeddings(texts, None)
                embed_list.append(embed.squeeze(0).numpy().tolist())
            elif data[1] == gate_to_index['NOT']:  
                texts = generate_not_prompt()
                embed = get_embeddings(texts, None)
                embed_list.append(embed.squeeze(0).numpy().tolist())
            else:
                embed_list.append([0.0] * 4096)
        ...
        labels[circuit_name] = {
            ...
            'h_init':np.array(embed_list).astype('float32'),
        }
