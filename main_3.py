...
from datasets.mlpgate_dataset import MLPGateDataset, MLPGateDataset2, MLPGateDataset3
...
def main(args):
    ...
    print("we are using {} {}in this run".format(args.device, args.gpus_str))
    args.world_size = 1
    ...
    if args.device != 'cpu' and len(args.gpus) > 1:
        ...
        args.device = 'cuda:{}'.format(args.gpus[0]) if args.gpus[0] >= 0 else 'cpu'
    if args.distributed:
    ...
    dataset = MLPGateDataset3(args.data_dir, args)
    ...
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers,
                            sampler=val_sampler)
    finite_list_full = dataset.finite_list_list
    trans_matrics = dataset.trans
    ...
    if args.val_only:
        ...
        print(len(val_dataset), ' validation batches')
        log_dict_val, _ = trainer.val(0, val_loader,args.local_rank, finite_list_full, trans_matrics)
    ...
    for epoch in range(start_epoch + 1, args.num_epochs + 1):
        ...
        log_dict_train, _ = trainer.train(epoch, train_loader, args.local_rank, finite_list_full, trans_matrics)
        if args.local_rank == 0:
        ...
        with torch.no_grad():
            ...
            log_dict_val, _ = trainer.val(epoch, val_loader, args.local_rank, finite_list_full,trans_matrics)
