...
def get_parse_args():
    ...
    parser.add_argument('--exp_id', default='pre-training')
    ...
    parser.add_argument('--save_intervals', type=int, default=1,
    ...
    # dataset settings
    parser.add_argument('--no_rc', default=True, action='store_true')
    parser.add_argument('--data_dir', default='../data/benchmark_circuits',
    ...
    parser.add_argument('--h_init_path', default='/home/jingyi/workspace/DeepSeq2-ICCAD/DeepSeq2-ICCAD/exp/stage1_embedding.json', help='Reload initial feature')
    ...
    # model settings
    parser.add_argument('--arch', default='mlpgnn', choices=['recgnn', 'convgnn', 'dagconvgnn', 'mlpgnn', 'mlpgnn_merge', 'mlpgnn_dg2','mlpgnn_pe','mlpgnn_stage2', 'mlpgnn_ds2','baseline','graphGPS'],
    ...
    parser.add_argument('--modelname', default='GCN', type=str, choices=['GCN', 'GAT', 'GraphSAGE','GIN','SGFormer','DIFFormer'],
                             help='The activation function of baseline GNN.')  
    ...
    parser.add_argument('--dg2_path', default='../pretrained/model.pth', type=str,
                             help='The path to the pretrained model of DG2.')   
    #baseline
    parser.add_argument('--randomwalk', default=3, type=int,
                             help='the number of random walk steps to generate positional embedding.')

    # recgnn 
    parser.add_argument('--num_aggr', default=3, type=int,
                             help='the number of aggregation layers.')
    parser.add_argument('--aggr_function', default='tfmlp', type=str, choices=['deepset', 'aggnconv', 'gated_sum', 'conv_sum', 'mlp', 'attnmlp', 'tfmlp','GraphSAGE'],
    ...
    parser.add_argument('--lt_ff', action='store_true', default=False,
                             help='Linear transformation for FF or normall aggregation and update operation.')  
    ...
    parser.add_argument('--prob_weight', type=float, default=2)
    parser.add_argument('--rc_weight', type=float, default=0)
    parser.add_argument('--trans_weight', type=float, default=2)
    parser.add_argument('--seq_weight', type=float, default=0)
    parser.add_argument('--func_weight', type=float, default=3)
    ...
    parser.add_argument('--test_data', default = False, action='store_true', 
                             help='data consists of large circuits for inference.')
    
    parser.add_argument('--stage', type = int, default=1, 
                             help='training stage of DeepSeq2, {1 or 2}')
    ...
    args.gate_to_index = {'INPUT': 0, 'AND': 1, 'NOT': 2, 'DFF': 3, 'PDFF': 4, 'NAND': 5, 'NOR': 6, 'OR': 7, 'XOR': 8, 'BUF': 9}
    ...
    if args.task in ['prob', 'diff']:
        assert args.arch in ['recgnn', 'convgnn', 'dagconvgnn', 'mlpgnn', 'mlpgnn_dg2', 'mlpgnn_pe', 'mlpgnn_stage2', 'mlpgnn_ds2','baseline','graphGPS'], 'The architecture should be either recgnn or convgnn, if the task is either prob or diff.'
