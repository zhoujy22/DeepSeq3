...
class MLPGate(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args.aggr_function == 'mlp' or args.aggr_function == 'attnmlp':
            ...
            self.aggr_not_seq = _aggr_function_factory[args.aggr_function](self.dim_hidden*1, args.dim_mlp, self.dim_hidden, num_layer=3, act_layer='relu')
    ...
    def forward(self, G,batch_size,nodes_in_cycles):
        ...
        preds = self._gru_forward(G, hs_init, hf_init,hseq_init, num_layers_f, num_layers_b,batch_size,nodes_in_cycles)
        
        return preds, max_sim
            
    def _gru_forward(self, G, hs_init, hf_init, hseq_init, num_layers_f, num_layers_b,batch_size,nodes_in_cycles ,h_true=None, h_false=None):
        G = G.to(self.device)
                ...
                if len(l_ff_fanin_node) > 0:           
                    ...
                    for node in ff_fanin_edge_index[1]:       
                        if cyclic_FFs_nodes[node]:
                            ...
                            len_circuit = (len(G.x)/batch_size)
                            
                            node_pos = node - G.batch[node]*len_circuit
                            start_indx= int(G.batch[node] * len_circuit)
                            end_indx = start_indx  + len_circuit 
                    
                            if end_indx - start_indx >= len_circuit:
                                nodes_in_path[start_indx:int(end_indx)] += nodes_in_cycles[node_pos.long(),:]  
        ...
        hseq = node_embedding_seq[:, self.dim_hidden + self.dim_hidden:]
       ...
       # return hs, hf, hseq, prob, trans_prob, is_rc, 
        return hs, hf, hseq, prob, trans_prob, None, 
