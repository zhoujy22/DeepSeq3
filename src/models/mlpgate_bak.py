...
class MLPGate(nn.Module):
    ...
    def _gru_forward(self, G, hs_init, hf_init, hseq_init, num_layers_f, num_layers_b, h_true=None, h_false=None):
        ...
        x, edge_index, ff_fanin_edge_index, original_edge_index, cyclic_FFs_nodes = G.x, G.edge_index, G.ff_fanin_edge_index , G.original_edge_index, G.cyclic_FFs_nodes[0]
        ...
        for _ in range(self.num_rounds):
            for level in range(1, num_layers_f):
                ...
                if l_and_node.size(0) > 0:
                    ...
                    msg = self.aggr_and_seq(node_state, and_edge_index, and_edge_attr) #function or structure or sequential?
                    and_msg = torch.index_select(msg, dim=0, index=l_and_node)
                ...
                # Step# 1: check if the current nodes are fan-ins of FFs
                src = ff_fanin_edge_index[0][0]
                dst = ff_fanin_edge_index[0][1]

                for n in l_and_node: #TODO: Stone, double check the code here and help to remove the loop if possible to optimize the code
                    ...
                    ff_fanin_state = torch.index_select(hf, dim=0, index=src_nodes.long())
                    hf[dest_ff.long(), :]  = self.linear_transform_ff_func(ff_fanin_state)


                    ## FF sequential embeddings
                    #linear transformation of representation at FF to induce clock delay
                    ff_fanin_state =  torch.index_select(hseq, dim=0, index=src_nodes.long())
                    hseq[dest_ff.long(),:] = self.linear_transform_ff_seq(ff_fanin_state)
                

                    # Updating cycle of FF, if any
                    if len(dest_ff) > 0:
                        if int(cyclic_FFs_nodes[int(dest_ff)]) == 1:
                            print("FF causing cycle", dest_ff)
                            # getting all nodes in cyclic graph 
                
                            visited = [False] * len(x)
                            nodes_in_path = []
                            edges_in_cycle = []
                            cyclic_nodes = [False] * len(x)
                           
                            result, nodes_in_path = find_nodes_in_cycle(int(dest_ff), int(dest_ff), original_edge_index, visited, nodes_in_path, flag= True)
                            ...
                            if result:
                                ...
                                for ind in range(len(original_edge_index[0])):
                                 
                                    if original_edge_index[0][ind] in nodes_in_path and original_edge_index[1][ind] in nodes_in_path:
                                        print(original_edge_index[0][ind], original_edge_index[1][ind], nodes_in_path)
                                        edges_in_cycle.append([original_edge_index[0][ind],original_edge_index[1][ind]])
                            ...
                            print("cylic node mask: ", cyclic_nodes)
                            print("edges_in_cycle ",edges_in_cycle)
                            cyclic_nodes = torch.tensor(cyclic_nodes, dtype=torch.bool)

                            """
                            Start of cycle processing
                            """
                            for c_level in range(1, num_layers_f):
                                ...
                                # AND Gate
                                c_l_and_node = G.forward_index[c_layer_mask & and_mask & cyclic_nodes]
                                if c_l_and_node.size(0) > 0:
                                    c_and_edge_index, c_and_edge_attr = subgraph(c_l_and_node, edges_in_cycle, edge_attr, dim=1)
                                    # Update structure hidden state
                                    ...
                                    # Update sequential hidden state
                                    c_msg = self.aggr_and_seq(node_state, c_and_edge_index, c_and_edge_attr) #function or structure or sequential?
                                    c_and_msg = torch.index_select(c_msg, dim=0, index=c_l_and_node)
                                ...
                                # NOT Gate
                                l_not_node = G.forward_index[c_layer_mask & not_mask & cyclic_nodes]
                                if c_l_not_node.size(0) > 0:
                                    c_not_edge_index, c_not_edge_attr = subgraph(c_l_not_node, edges_in_cycle, edge_attr, dim=1)
                                    # Update structure hidden state
                                ...
                                for n in l_and_node:
                                    ne_idx = ff_fanin_edge_index[0][0] == n
                                    if len(ff_fanin_edge_index[0][1][ne_idx])!= 0 and ff_fanin_edge_index[0][1][ne_idx] in nodes_in_path:
                                ...
                                if le_idx != []:
                                    ...
                                    lp_edge_index = ff_fanin_edge_index[0][:, le_idx]
                                
                                
                                    ff_fanin_state = torch.index_select(hf, dim=0, index=lp_edge_index[0].long())
                                    hf[lp_edge_index[1], :]  = self.linear_transform_ff_func(ff_fanin_state)

                                    ff_fanin_state = torch.index_select(hseq, dim=0, index=lp_edge_index[0].long())
                                    hseq[lp_edge_index[1], :]  = self.linear_transform_ff_seq(ff_fanin_state)
                    ...
                    if len(dest_ff) > 0:
                        if int(cyclic_FFs_nodes[int(dest_ff)]) == 1:
                            ...
                            for c_level in range(1, num_layers_f):
                                ...
                                # NOT Gate
                                c_l_not_node = G.forward_index[c_layer_mask & not_mask & cyclic_nodes]
                                ...
                                for n in c_l_and_node:
                                    ne_idx = ff_fanin_edge_index[0][0] == n
                                    if len(ff_fanin_edge_index[0][1][ne_idx])!= 0 and ff_fanin_edge_index[0][1][ne_idx] in nodes_in_path:
                                            le_idx += [ne_idx.nonzero().squeeze(-1)]

                                for n in c_l_not_node:
                                    ne_idx = ff_fanin_edge_index[0][0] == n
                                    if len(ff_fanin_edge_index[0][1][ne_idx])!= 0 and  ff_fanin_edge_index[0][1][ne_idx] in nodes_in_path:
                                        le_idx += [ne_idx.nonzero().squeeze(-1)]
        ...
        msg = self.aggr_ff_strc(hs, ff_fanin_edge_index[0])
