...
def map_node_ids(edge_index):
    ...
    idx_to_node_id = {i: nid for i, nid in enumerate(node_ids)}

    # 重新映射所有边
    edge_index_mapped = torch.tensor([
        [node_id_to_idx[s] for s in src],
        [node_id_to_idx[t] for t in tgt]
    ], dtype=torch.long)

    return edge_index_mapped, node_id_to_idx, idx_to_node_id
...
def subgraph(target_idx, edge_index, edge_attr=None, dim=0):
    ...
    function from DAGNN
    '''
    mask = torch.isin(edge_index[dim], target_idx)
    
    lp_edge_index = edge_index[:, mask]
    
    if edge_attr is not None:
        lp_edge_attr = edge_attr[mask]
    else:
        lp_edge_attr = None
