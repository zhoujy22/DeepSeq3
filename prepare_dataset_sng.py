...
def get_parse_args():
    ...
    parser.add_argument('--exp_id', default='train_stage2')
...
if __name__ == '__main__':
    ...
    for bench_filename in name_list[args.start_idx: min(args.end_idx, len(name_list))]:
        ...
        for idx, x in enumerate(x_data):
            if x[1] == gate_to_index['DFF'] or x[1] == gate_to_index['OUTPUT']:
        ...
        y = [0] * len(x_data_dff) # 逻辑1概率
        index = 0
        for idx in range(len(x_data)):
            if x_data[idx][1] == gate_to_index['DFF']:
                y[index] = np.sum(tt[idx]) / len(tt[idx])
                index += 1
        number = pow(2, len(x_data_dff))
        x_data_py = [(int(a), int(b)) for a, b, _ in x_data]
        trans_matric = fsm_simulator.simulate_one_step_matrix(
                                                            x_data=x_data_py,
        ...
        finite_list = fsm_simulator.reachability_closure(
                                                            x_data=x_data_py,
                                                            PI_indexes=[int(x) for x in PI_index],
                                                            level_list=[[int(y) for y in level] for level in level_list],
                                                            fanin_list=[[int(y) for y in fanin] for fanin in fanin_list],
        ...
        for idx in range(len(x_data)):
            if x_data[idx][1] == gate_to_index['DFF']:
                tt_dff[index] = tt[idx]
                index += 1
        t_01, t_10 = estimate_transition_prob_from_tt(tt_dff)

        # Save 
        x_data = utils.rename_node(x_data)
        graphs[circuit_name] = {'x': np.array(x_data_dff).astype('float32'), "edge_index": np.array(edge_index_data)}
        labels[circuit_name] = {
            'prob_logic1': np.array([y]).astype('float32'),
            'tt_01': np.array(t_01).astype('float32'),'tt_10': np.array(t_10).astype('float32'),
            'finite_list':np.array(finite_list[0]).astype('float32'),
            'trans_matric':np.array(trans_matric[0]).astype('float32'),
        }
