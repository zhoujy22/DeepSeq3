...
from datasets.finetuning_pe_dataset import MLPGateFinetuningPE
...
def main(args):
    ...
    dataset = MLPGateFinetuningPE(args.data_dir, args)
    ########################################################################### !!!!
    # Debug
    ...
    """
    G = train_dataset[0]
    nodes_in_cycles = torch.ones(len(G.x), len(G.x))
    nodes_in_cycles  = nodes_in_cycles.fill_(False)
        
    one_path = torch.tensor([False] * len(G.x))
    cycle_path = torch.tensor([False] * len(G.x))
    for node in G.ff_fanin_edge_index[1]:         
        visited = [False] * len(G.x)
        cycle_path = torch.tensor([False] * len(G.x))
        if G.cyclic_FFs_nodes[node]:
                cycle_path = find_nodes_in_cycle(int(node), int(node), G.original_edge_index, visited, one_path, cycle_path)
             
                nodes_in_cycles[node,:] += cycle_path
    

    nodes_in_cycles = nodes_in_cycles.bool()
    ...
    if args.val_only:
        log_dict_val, _ = trainer.val(0, val_loader,args.local_rank,args.batch_size, args.nodes_in_cycles)
    ...
    for epoch in range(start_epoch + 1, args.num_epochs + 1):
        ...
        log_dict_train, _ = trainer.train(epoch, train_loader, args.local_rank,args.batch_size, nodes_in_cycles)
        if args.local_rank == 0:
        ...
        with torch.no_grad():
            ...
            log_dict_val, _ = trainer.val(epoch, val_loader, args.local_rank,args.batch_size, nodes_in_cycles)
